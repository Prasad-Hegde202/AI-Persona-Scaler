...
// Safely convert whatever the backend returns into a plain string
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function normalizeAnswer(raw: any): string {
  if (typeof raw === "string") return raw;
  if (raw == null) return "";
  // e.g. { text: "..." } or { answer: "..." } or { content: "..." }
  if (typeof raw === "object") {
    return (
      raw.text ?? raw.answer ?? raw.content ?? raw.message ?? JSON.stringify(raw)
    );
  }
  return String(raw);
}

// Safely convert sources into string[]
// Handles: string[], {repo,type}[], mixed arrays, nested arrays
// eslint-disable-next-line @typescript-eslint/no-explicit-any
...
export default function Home() {
  ...
  const sendMessage = async (text?: string) => {
    ...
    try {
      ...
      setMessages((prev) => [...prev, {
        role: "assistant",
        text: "Could not reach the backend. Please check if the server is running.",
        time: getTime(),
      }]);
    }
    setLoading(false);
  ...
  const t = dark ? {
    ...
    meta: "text-[#4E5872]",
    metaLink: "text-[#4E5872] hover:text-[#8892A4]",
  } : {
    page: "bg-[#F0F2F7]",
    ...
    meta: "text-[#9BA3B8]",
    metaLink: "text-[#9BA3B8] hover:text-[#5A6478]",
  };

  return (
    <main className={`min-h-screen ${t.page} flex justify-center items-start p-4 md:p-6 transition-colors duration-200`}>
      <div className={`w-full max-w-3xl ${t.shell} rounded-2xl border flex flex-col overflow-hidden transition-colors duration-200`}
        ...
        <div className={`flex items-center gap-3 px-5 py-3.5 border-b ${t.header} flex-shrink-0`}>
          <div className="flex-1 min-w-0">
            <h1 className={`font-semibold text-[15px] ${t.title} leading-tight`}>Prasad Hegde — AI Persona</h1>
            <p className={`text-[12.5px] ${t.sub} mt-0.5`}>Resume · Projects · GitHub · Skills </p>
          ...
          {messages.map((msg, i) => (
            <div key={i} className={`flex gap-2.5 ${msg.role === "user" ? "flex-row-reverse" : ""}`}>
              ...
              <div className={`max-w-[76%] flex flex-col gap-1.5 ${msg.role === "user" ? "items-end" : "items-start"}`}>
                <div
  className={`px-3.5 py-2.5 rounded-2xl text-[14px] leading-relaxed whitespace-pre-line ${
    msg.role === "user"
      ? `${t.bubbleUser} rounded-br-sm`
      : `${t.bubbleBot} rounded-bl-sm`
  }`}
>
  {msg.text}
</div>
        ...
        <div className={`px-4 py-3 border-t ${t.footer} flex-shrink-0 transition-colors duration-200`}>
          <div className="flex gap-2 items-center">
            ...
            <button onClick={() => sendMessage()} disabled={loading || !question.trim()}
              className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 transition-all active:scale-95 disabled:cursor-not-allowed ${!loading && question.trim() ? t.sendActive : t.sendDisabled}`}
              aria-label="Send">
